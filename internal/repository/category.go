...
func (r *CategoryRepo) UpdateCategory(ctx context.Context, categoryID int, newData map[string]interface{}) error {
	err := r.DB.Debug().Model(&models.ProductCategory{}).Where("id = ?", categoryID).Updates(newData).Error
	if err != nil {
		return err
	}
...
func (r *CategoryRepo) GetCategory(ctx context.Context) ([]models.ProductCategory, error) {
	var (
